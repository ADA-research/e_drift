...
def retrain_network(retraining_features, retraining_labels, model):
    ...
    #save model
    torch.save(model.state_dict(), f"models/model_weights/electricity.pth")
...
def get_new_predictions(features, model):
    ...
    np.save(database_path + f"/datasets/labels_electricity_pred.npy", new_predictions)
...
def main():
    ...
    #retrieve model
    model = Net_7_2()
    model.load_state_dict(torch.load(f"models/model_weights/{dataset_name}.pth"))

    #yaml file name
    yaml_name = f"exp_configs/tutorial_examples/custom_EL.yaml"
    result_file = f'custom_EL.txt'
